          ...
          <div class="content-wrapper">
            <div class="row">
              ...
              <div class="col-lg-12 grid-margin stretch-card">
                <div class="card">
                  <div class="card-body">
                  <a href="{{route('admin.addDeliveryMode')}}" class="btn btn-outline-info btn-fw" style="float: right;">Add Delivery Mode</a>
                    <h4 class="card-title">Delivery Mode Management</h4>
                    <p class="card-description"> Delivery Mode List 
                    </p>
                    <form id="bulkDeleteForm" method="POST" action="{{ route('admin.bulkDeleteMode') }}">
                      @csrf
                        ...
                        <table class="table table-striped" id="example">
                          ...
                          <tbody>
                            ...
                            <tr>
                              ...
                              <td>                              
                                <a href="{{route('admin.addDeliveryMode')}}/{{ $list->id }}"><i class="mdi mdi-lead-pencil"></i></a></td>
                            </tr>
                            @php $i++; @endphp 
                            @endforeach
                            @endif
                    ...
                    <div class="d-flex add-pagination mt-4">
                        {{ $delivery->links('pagination::bootstrap-4') }}
                    </div>
                  </div>
                </div>
              </div>
        ...
        <script>
          ...
          $(document).ready(function () {
            $(document).on('change','.user_status',function(){
              ...
              $.ajax({
                url: "{{url('/admin/update_mode_status')}}",
                type: "POST",
                datatype: "json",
                data: {
                  status: status,
                ...
                success: function(result) {
                  Swal.fire({
                    title: "Success!",
                    text: "Delivery Mode Status updated!",
                    icon: "success"
                  });
                },
                errror: function(xhr) {
          ...
          document.getElementById('bulkDeleteBtn').addEventListener('click', function (e) {
            ...
            if (checkboxes.length === 0) {
              Swal.fire({
                icon: 'warning',
                title: 'No selection',
                text: 'Please select at least one Delivery Mode to delete.',
              });
              return;
            }
            ...
            Swal.fire({
              title: 'Are you sure?',
              text: "Selected Delivery Mode will be deleted.",
              icon: 'warning',
