    ...
    font-size: 18px;
          ...
          <div class="content-wrapper">
            <div class="row">
              ...
              <div class="col-lg-12 grid-margin stretch-card">
                <div class="card">
                  <div class="card-body">
                  <a href="{{route('admin.addSubscription')}}" class="btn btn-outline-info btn-fw" style="float: right;">Add Subscription Plan</a>
                    <h4 class="card-title">Subscription Plan Management</h4>
                    <p class="card-description"> Subscription Plan List 
                    </p>
                    
                    <form id="bulkDeleteForm" method="POST" action="{{ route('admin.bulkDeletePlan') }}">
                      @csrf
                        ...
                        <table class="table table-striped" id="example">
                          ...
                          <tbody>
                            ...
                            <tr>
                              ...
                              <td> {{$list->plan_duration}} -

                              {{ $list->duration_unit == 1 ? 'Day' : ($list->duration_unit == 2 ? 'Month' : ($list->duration_unit == 3 ? 'Year' : '')) }}
                              ...
                              <td>                              
                                <a href="{{route('admin.addSubscription')}}/{{ $list->id }}"><i class="mdi mdi-lead-pencil"></i></a></td>
                            </tr>
                            @php $i++; @endphp 
                            @endforeach
                            @endif
        ...
        <script>
          ...
          $(document).ready(function () {
            $(document).on('change','.user_status',function(){
              ...
              $.ajax({
                url: "{{url('/admin/update_subscri_status')}}",
                type: "POST",
                datatype: "json",
                data: {
                  status: status,
                ...
                success: function(result) {
                  Swal.fire({
                    title: "Success!",
                    text: "Subscription Plan Status updated!",
                    icon: "success"
                  });
                },
                errror: function(xhr) {
        ...
        <script>
          ...
          document.getElementById('bulkDeleteBtn').addEventListener('click', function (e) {
            ...
            Swal.fire({
              title: 'Are you sure?',
              text: "Selected Plan will be deleted.",
              icon: 'warning',
