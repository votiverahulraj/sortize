              ...
              <div class="col-lg-12 grid-margin stretch-card">
                <div class="card">
                  ...
                  <a href="{{route('admin.addPolicy')}}" class="btn btn-outline-info btn-fw" style="float: right;">Add Policy</a>
                    ...
                    <form id="bulkDeleteForm" method="POST" action="{{ route('admin.bulkDeletePolicy') }}">
                      @csrf
            ...
            $(document).on('click','.del_policy',function(){
              ...
              swalWithBootstrapButtons.fire({
                ...
                if (result.isConfirmed) {
                  ...
                  var policy_id=$(this).attr('user_id');
                  $.ajax({
                    url: "{{url('/admin/deletePolicy')}}",
                    type: "POST",
                    datatype: "json",
                    data: {
                      policy_id:policy_id,
          ...
          document.getElementById('bulkDeleteBtn').addEventListener('click', function (e) {
            ...
            if (checkboxes.length === 0) {
              Swal.fire({
                icon: 'warning',
                title: 'No selection',
                text: 'Please select at least one policy to delete.',
              });
              return;
            }
            ...
            Swal.fire({
              title: 'Are you sure?',
              text: "Selected policies will be deleted.",
              icon: 'warning',
              showCancelButton: true,
